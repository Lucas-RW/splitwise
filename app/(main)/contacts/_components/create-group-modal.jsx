...
const groupSchema = z.object({
  name: z.string().min(1, "Gruop name is required"),
...
const CreateGroupModal = ({ isOpen, onClose, onSuccess }) => {
  ...
  const { register, handleSubmit, formState:{ errors, isSubmitting }, reset } = useForm({ resolver: zodResolver(groupSchema),
          defaultValues: {name: "", descriptrion: ""}
  });
                      ...
                      <CommandList>
                        <CommandEmpty>{searchQuery.length < 2 ? (
                          <p className="py-3 px-4 text-sm text-center text-muted-foreground">
                            Type at least 2 character to search
                          </p>
                        ) :  isSearching ? (
                          <p className="py-3 px-4 text-sm text-center text-muted-foreground">
                            Searching...
